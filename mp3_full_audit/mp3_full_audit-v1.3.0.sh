...
########################
# Forgiving CSV Loader for Playnite Metadata (Git Bash Safe)
########################
load_playnite_csv() {
    local csv_file=""
    ...
    # Create a cleaned temporary copy of the CSV (CRLF → LF, no BOM)
    local tmp_csv
    tmp_csv=$(mktemp)
    sed 's/\r$//' "$csv_file" | sed '1s/^\xEF\xBB\xBF//' > "$tmp_csv"

    local is_header=1
    local line

    # Read from the cleaned file (no process substitution)

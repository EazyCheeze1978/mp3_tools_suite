#!/usr/bin/env python3
# ------------------------------------------------------------
# MP3 Reduce Tool — v0.2.3
# ------------------------------------------------------------
# Changes in 0.2.3:
# - Added WSL-safe input wrapper to prevent TTY freeze
# - Added ffmpeg version detection and display
# - Spinner remains disabled on WSL
# - Retained all 0.2.2 improvements and path normalization
# ------------------------------------------------------------
...
def detect_environment():
    ...
    if system == "linux" and is_wsl:
        ...
        return ffmpeg, ffprobe, "wsl", "WSL (Linux kernel under Windows)"

    if system == "windows":
        ffmpeg = shutil.which("ffmpeg")
        ffprobe = shutil.which("ffprobe")
        if ffmpeg and ffprobe:
            return ffmpeg, ffprobe, "windows", "Windows (native Python)"
        raise RuntimeError("ffmpeg/ffprobe not found on Windows PATH")

    ffmpeg = shutil.which("ffmpeg")
    ffprobe = shutil.which("ffprobe")
    if ffmpeg and ffprobe:
        return ffmpeg, ffprobe, "linux", f"{base_desc} (native)"
...
# ------------------------------------------------------------
# SECTION: WSL-Safe Input Wrapper
# ------------------------------------------------------------
def safe_input(prompt, mode):
    print("", flush=True)
    if mode == "wsl":
        time.sleep(0.1)
    return input(prompt)
...
# ------------------------------------------------------------
# SECTION: Directory Input Normalization
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# SECTION: Path Normalization for ffmpeg
# ------------------------------------------------------------
...
def reduce_worker(item):
    ...
    cmd = [
        item["ffmpeg_cmd"],
        "-y",
        "-i", normalize_path(src, item["mode"]),
        "-vn",
        "-acodec", "libmp3lame",
        "-b:a", "128k",
        normalize_path(dst, item["mode"]),
    ]
...
# ------------------------------------------------------------
# SECTION: Spinner (Disabled on WSL)
# ------------------------------------------------------------
...
def main():
    ...
    # ffmpeg version
    version_line = get_ffmpeg_version(ffmpeg_cmd)
    print(f"ffmpeg version: {version_line}\n")

    # argparse
    parser = argparse.ArgumentParser(
        description="MP3 Reduce Tool — v0.2.3 (Python Port)"
    )
    ...
    if args.dir:
        ...
        raw_dir = safe_input("Enter directory to scan: ", mode).strip('"')
    ...
    # Reduction confirmation (WSL-safe)
    choice = safe_input("\nProceed with reduction? (y/n): ", mode).strip().lower()
    if choice != "y":
        print("Aborted.")
        ...
        return
    # ------------------------------------------------------------
    # Parallel reduction (spinner disabled on WSL)
    # ------------------------------------------------------------
    print("\nStarting parallel reduction...\n")

    counter = {"count": 0}
    stop_event = threading.Event()
    use_spinner = (mode != "wsl")

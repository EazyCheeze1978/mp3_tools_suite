...
# ------------------------------------------------------------
# Detect platform + ffmpeg availability
# ------------------------------------------------------------
def detect_ffmpeg():
    """
    Returns a tuple:
    (ffmpeg_command, ffprobe_command, mode)
    ...
    if system == "windows":
        ...
        if ffmpeg and ffprobe:
            return ffmpeg, ffprobe, "windows"

        # If not found, try WSL fallback
        if shutil.which("wsl"):
            return "wsl ffmpeg", "wsl ffprobe", "wsl"

        raise RuntimeError("ffmpeg not found on Windows or WSL")

    # Linux / WSL
    ffmpeg = shutil.which("ffmpeg")
    ffprobe = shutil.which("ffprobe")
    if ffmpeg and ffprobe:
        return ffmpeg, ffprobe, "linux"

    raise RuntimeError("ffmpeg not found on Linux/WSL")


# ------------------------------------------------------------
# Normalize paths for Windows / WSL
# ------------------------------------------------------------
def normalize_path(path: Path, mode: str):
    """
    Ensures paths are safe for ffmpeg depending on platform.
    """
    if mode == "windows":
        return str(path)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MP3 Reduce Tool — v0.1.5 (Python Port)"
    )
    ...
    # -----------------------------
    # Detect ffmpeg + platform
    # -----------------------------
    ffmpeg_cmd, ffprobe_cmd, mode = detect_ffmpeg()

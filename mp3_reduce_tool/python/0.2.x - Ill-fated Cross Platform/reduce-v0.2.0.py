#!/usr/bin/env python3
# ------------------------------------------------------------
# MP3 Reduce Tool — v0.2.0
# Windows Path Normalization & Compatibility Polish
#
# Major features:
# - Robust OS detection (Windows / WSL / Linux)
# - Full path normalization for ffmpeg/ffprobe
# - Safe deletion workflow with verification + prompt
# - Skip logic for existing _reduced files
# - Improved logging + CSV timestamping
...
def detect_environment():
    ...
    # Detect WSL (Linux kernel but with /mnt/c and WSL env)
    is_wsl = False
    try:
    ...
    if system == "windows":
        ...
        # If ffmpeg not found, we fail explicitly
        raise RuntimeError("ffmpeg/ffprobe not found on Windows PATH")

    # Plain Linux / other Unix
    ffmpeg = shutil.which("ffmpeg")
    ffprobe = shutil.which("ffprobe")
    if ffmpeg and ffprobe:
...
def normalize_path(path: Path, mode: str) -> str:
    ...
    - Linux POSIX paths
    """
    # Resolve environment variables and relative segments
    expanded = os.path.expandvars(str(path))
    resolved = str(Path(expanded).resolve())

    # UNC paths (Windows network shares)
    if resolved.startswith("\\\\"):
        # Convert \\server\share\folder → //server/share/folder
        resolved = resolved.replace("\\", "/")
        return resolved

    if mode == "windows":
        # ffmpeg on Windows is generally fine with either backslashes or slashes,
        # but forward slashes are safer and more consistent.
        resolved = resolved.replace("\\", "/")
        return resolved

    if mode == "wsl":
        # Convert Windows-style paths to /mnt/<drive>/...
        # Example: C:\Users\Me\Music → /mnt/c/Users/Me/Music
        # If it's already a POSIX path, just return it.
...
def reduced_exists(path: Path) -> bool:
    """
    Returns True if a corresponding _reduced.mp3 file exists.
    Example:
        track.mp3 → track_reduced.mp3
    """
...
def write_csv(rows, path: Path):
    """
    Writes the CSV report with consistent column order.
    """
    if not rows:
        return
...
def ffprobe_audio_info(path: Path, ffprobe_cmd: str, mode: str):
    """
    Runs ffprobe to extract bitrate, duration, and size.
    Returns None if metadata is missing or unreadable.
    """
    try:
        cmd = [
            ffprobe_cmd,
...
def reduce_worker(item):
    """
    Worker process:
    - Runs ffmpeg to create _reduced.mp3
    - Does NOT delete originals (handled later)
    - Returns metadata for logging/CSV
    """
    src = item["path"]
...
def verify_and_delete(original: Path, reduced: Path, ffprobe_cmd: str, mode: str, log):
    """
    Deletes the original file ONLY if:
    - reduced exists
    - reduced is non-zero
    - reduced is smaller
    - reduced is newer
    - reduced passes ffprobe validation
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MP3 Reduce Tool — v0.2.0 (Python Port)"
    )
    ...
    # Map from src path string → info (for later CSV/log use)
    file_info = {}
    ...
    for mp3 in music_dir.rglob("*.mp3"):
        ...
        # Skip if reduced version already exists
        ...
        # Already reduced by name (fallback)

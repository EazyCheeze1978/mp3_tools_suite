...
# - Savings limiter (--force-savings) restores "skip <20 percent savings"
...
# ------------------------------------------------------------
def verify_and_delete(original: Path, reduced: Path, ffprobe_cmd: str, log, allow_bigger: bool = False):
    try:
        ...
        if not allow_bigger and reduced.stat().st_size >= original.stat().st_size:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "MP3 Reduce Tool — v0.3.3 (Python Port, Windows Only)\n\n"
    ...
    # Optional limiters (disabled by default)
    parser.add_argument(
        "--force-bitrate",
        action="store_true",
    ...
    parser.add_argument(
        "--force-savings",
        action="store_true",
        help="Enable savings limiter (skip files with less than 20 percent estimated savings)."
    )
    ...
    if apply_savings_limit:
        print("Savings limiter ENABLED (skip with less than 20 percent estimated savings).")
    else:
    ...
    # ------------------------------------------------------------
    # PREVIEW LOOP (with optional limiters)
    # ------------------------------------------------------------
    ...
    for mp3 in discovered_files:
        ...
        # OPTIONAL LIMITER: bitrate threshold
        ...
        # OPTIONAL LIMITER: savings threshold
        ...
        log(f"READY: {mp3.name} ({round(savings_percent, 2)} percent savings est.)")
    ...
    # ------------------------------------------------------------
    # POST-PROCESSING: SUMMARY OF RESULTS
    # ------------------------------------------------------------
    successful = [r for r in results if r["success"]]
    failed = [r for r in results if not r["success"]]

    bigger = [r for r in successful if r["savings_bytes"] <= 0]
    normal = [r for r in successful if r["savings_bytes"] > 0]

    print("Reduction summary:")
    print(f"  Successful reductions: {len(successful)}")
    print(f"  Failed conversions:    {len(failed)}")
    print(f"  Reduced but larger:    {len(bigger)} (negative or zero savings)")
    print(f"  Reduced and smaller:   {len(normal)}\n")

    if bigger:
        print("Note: Some reduced files are larger than the originals.")
        print("They will NOT have their originals deleted unless you explicitly allow it.\n")

    # ------------------------------------------------------------
    # POST-PROCESSING: SAFE DELETION (WITH PROMPTS)
    # ------------------------------------------------------------
    proceed_delete = True

    if not args.auto:
        ans = input("Delete originals for successfully reduced files? (y/n): ").strip().lower()
        if ans != "y":
            proceed_delete = False

    if proceed_delete:
        print("\nVerifying and deleting originals...\n")

        # First: delete only those with positive savings
        for r in normal:
            original = Path(r["src"])
            reduced = Path(r["dst"])
            verify_and_delete(original, reduced, ffprobe_cmd, log, allow_bigger=False)
        ...
        # Then: optionally handle "bigger" files
        allow_bigger_delete = False

        if bigger:
            if not args.auto:
                ans_bigger = input(
                    "Some reduced files are larger than the originals.\n"
                    "Delete originals for those as well? (y/n): "
                ).strip().lower()
                if ans_bigger == "y":
                    allow_bigger_delete = True
            else:
                # In auto mode, do NOT delete originals for bigger files
                allow_bigger_delete = False

            if allow_bigger_delete:
                print("\nDeleting originals for larger reduced files as requested...\n")
                for r in bigger:
                    original = Path(r["src"])
                    reduced = Path(r["dst"])
                    verify_and_delete(original, reduced, ffprobe_cmd, log, allow_bigger=True)
                    ...
                    csv_rows.append({
                        ...
                        "savings_percent": round(r["savings_percent"], 2),
                        "action": "REDUCED",
                        "skip_reason": "original larger but deleted by user choice",
                    })
            else:
                # Log that we skipped deletion for these
                for r in bigger:
                    original = Path(r["src"])
                    log(f"DELETE SKIP (reduced larger; user did not allow delete): {original}")
                    csv_rows.append({
                        "filename": original.name,
                        "bitrate": "",
                        "size_bytes": "",
                        ...
                        "savings_percent": round(r["savings_percent"], 2),
                        "action": "REDUCED",
                        "skip_reason": "original kept (reduced larger)",
                    })
    else:
        print("Original files were not deleted (per user choice or auto mode).\n")
        # Still record successful reductions in CSV
        for r in successful:
            original = Path(r["src"])
            csv_rows.append({
                "filename": original.name,

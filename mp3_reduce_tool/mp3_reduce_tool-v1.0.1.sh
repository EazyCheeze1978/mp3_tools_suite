...
# v1.0.1 — Parallelized Reduction Mode
#############################################
...
# -------- Helpers (unchanged except interactive pause removed) --------
...
# pause_if_interactive removed — incompatible with parallelization
...
# -------- NEW: Parallelized Reduction Mode --------
reduce_mode() {
    echo ""
    ...
    (
        while true; do
            ...
            PCT=$(( DONE * 100 / TOTAL ))
            echo -ne "Processed $DONE / $TOTAL ($PCT%)\r"
            sleep 1
        done
    ) &
    MONITOR_PID=$!
    ...
    xargs -P 8 -I {} bash -c 'reduce_one_file "$@"' _ {} < "$FILE_LIST"

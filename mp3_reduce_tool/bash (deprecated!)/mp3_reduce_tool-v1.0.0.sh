...
# - Works in current directory or optional target directory
# - Uses ffprobe/ffmpeg, stat, bc
#############################################
...
# Iterate over reducible files and call a callback function:
...
preview_mode() {
    ...
    # Final batch summary if not aligned to BATCH_SIZE
...
reduce_callback() {
    ...
    echo -e "${GREEN}Reducing:${RESET} $file"
    local dir base
    dir=$(dirname "$file")
    ...
    if [ $? -ne 0 ]; then
        echo -e "${RED}Failed to reduce:${RESET} $file"
    fi
}
...
reduce_mode() {
    ...
    iterate_reducible_files reduce_callback
...
safe_delete_mode() {
    ...
    while IFS= read -r file; do
        ...
        # Verify reduced file is valid audio

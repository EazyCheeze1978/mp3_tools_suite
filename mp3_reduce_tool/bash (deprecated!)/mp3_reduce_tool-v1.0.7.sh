...
# v1.0.7 — Subshell-safe + Null-safe + WSL-friendly
#############################################
...
# -------- Reduction Mode (null-safe, WSL-friendly) --------
reduce_mode() {
    echo ""
    ...
    # Filter to reducible files, build null-terminated list and TOTAL
    TOTAL=0
    ...
    if [ "$TOTAL" -eq 0 ]; then
        echo -e "${YELLOW}No files need reduction.${RESET}"
